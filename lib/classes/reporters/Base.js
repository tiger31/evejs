...
const Stacktrace = require('../../stack');
...
module.exports = class BaseReporter extends Reporter {
	...
	attach() {
		...
		this.runner.on(this.runner.constructor.events.EVENT_UNHANDLED_REJECTION, this._unhandled);
	}
	...
	_unhandled = (err) => {
		console.log(chalk.bold.yellow('Caught error outside the test environment. This is probably a problem with runner. If there is some unexpected behaviour please report it\n'), err);
	};
	...
	_test = (test) => {
		...
		if (test.state === Test.states.FAILED || test.state === Test.states.BROKEN) {
			...
			this.rendered.push(test.rendered);
		}
	...
	flushErrors = () => {
		for (let i = 0; i < this.errors.length; i++) {
			...
			const _trace = Stacktrace({forError: error}) || this.rendered[i];
			const trace = _trace.renderSync({stackFilter: this._filter, frameSize: 2});
