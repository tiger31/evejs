...
const Runner = class Runner extends Suite {
	...
	run = async () => {
		...
		this.driver.run();
		await this.framework.catch((error) => {
			this.emit(this.constructor.events.EVENT_STATE_FRAMEWORK_END, error);
			if (error instanceof error.MIFrameworkError) {
				this.error(error);
			} else {
				//TODO analysis
			}
	...
	exit = (error) => {
		...
		// noinspection JSUnresolvedFunction
		this.emit(this.constructor.events.EVENT_STATE_EMERGE);
...
Runner.events.EVENT_STATE_EMERGE = 'stateEmerge';
