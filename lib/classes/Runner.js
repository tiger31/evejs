...
const Runner = class Runner extends Suite {
	 ...
	 * @param {Function} fn - Function
	 */
	filter = (params = {}, parent, fn) => {
		for (const param of this.constructor.filters) {
			if (this.config.filters[param]) {
				if (params[param]) {
					if (parent[param] && params[param] !== parent[param])
						throw new errors.MITestsTreeError(param, params[param], parent[param]);
				} else params[param] = parent[param];
				if (this.config.filters[param] !== params[param]) {
					return;
				}
			}
		}
	 ...
	 * @static
	 */
	static filters = [ 'epic', 'feature', 'story', 'id' ];
