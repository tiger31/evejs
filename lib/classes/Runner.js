...
const Runner = class Runner extends Stateful(Suite) {
	...
	run = async () => {
		...
		//TODO Ordering
		for (const suite of this.MISuites.filter(s => s.config.mode === Suite.mode.ALLOW_TO_FAIL || s.seeded === Suite.seedingStatus.SUCCEEDED))
			await suite.run();
		this.driver.run();
	...
	static states = {
		init: new State('init', {seed: 'seed', error: 'error'}, 'error'),
		seed: new State('seed', {error: 'error'}, 'error'),
		error: new State('error', {emerge: 'emerge'}, 'emerge'),
		exit: new State('exit', {}, 'exit', true),
	}
};
