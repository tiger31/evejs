...
const EveReporter = require('./reporters/Eve');
...
const Runner = class Runner extends Stateful(Suite) {
	 ...
	 * It may be an error thrown by runner itself or unexpected error killing the runner process
	 * @type {Error}
	 */
	_error;
	//TODO runner config;
	...
	constructor(config = {}) {
		...
		this.reporter = reporters[this.config.reporter];
		if (!this.reporter)
			this.reporter = require(this.config.reporter);
		this.reporter = new this.reporter(this);
