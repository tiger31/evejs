...
const collector = require('../collectors').Suite;
...
const Suite = class Suite extends Collectable(Manager(Proxy(Exporter(Filterable(TaskRunnable(EventEmitter)))))) {
	...
	constructor({fn, title, runner, parent} = {}, {
		...
		this.exports = {
			...
			hooks: Object.values(hooks).reduce((o, h) => { o[h] = (fn, config) => {
				return this.registerHook(h, fn, config);
			}; return o; }, {})
		};
		if (init) {
			this.init();
	...
	/**
	 * @description
	 * Checks runnable state
	 * @param {TaskRunnable} task - Runnable to check state in
	...
	/**
	 * @description
	 * Checks Task result status
	 * If returns false - stops execution of left tasks
	 * @external Task
	...
	ignore() {
		...
		this.emit(events.SUITE_PENDING, this);
		this.emit(events.SUITE_BEGIN, this);
	...
	_stat = () => {
		...
		for (const suite of this.suites) {
			...
			for (const state of Object.values(Test.states)) {
