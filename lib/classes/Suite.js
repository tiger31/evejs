...
const Suite = class Suite extends TestSuite {
	...
	runSeeds = async () => {
		...
		const items = [];
		items.push({ func: this.launchHook, args: [this.constructor.hooks.BEFORE_SEEDS]});
		for (const seed of this.seeds) {
			items.push(
				{ func: this.launchHook, args: [this.constructor.hooks.BEFORE_SEED]},
				{ func: runnable.run, args: [this.context]},
				{ func: this.launchHook, args: [this.constructor.hooks.AFTER_SEED]},
			)
		}
		items.push({ func: this.launchHook, args: [this.constructor.hooks.AFTER_SEEDS]});
		...
		await this.runHook(this.constructor.hooks.BEFORE_SEEDS)
			...
			.catch(err => this._onHookFail(this.constructor.hooks.BEFORE_SEEDS, err));
		for (const seed of this.seeds) {
		...
		await this.runHook(this.constructor.hooks.AFTER_SEEDS)
			.then(this._onHookSuccess)
			.catch(err => this._onHookFail(this.constructor.hooks.AFTER_SEEDS, err));
		*/
	};
