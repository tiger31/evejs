 ...
 */
const Suite = class TestSuite extends Collectable(Manager(HookedRunnable)) {
	/**
	 * @type {Array.<Suite|Test>}
	 */
	runnables = [];
	/**
	 * @type {Array.<Error>}
	...
	get suites() {
		return this.getGroup().filter(r => r instanceof Suite);
	}
	...
	suite = (title, fn, config = {}) => {
		this.runner.filter(config, this.config,() => {
			const suite = new Suite({fn, name: title, runner: this.runner, context: this.context, parent: this}, config);
			this.proxy([
	...
	init = () => {
		...
		try {
			...
			this.runnables.forEach(r => { if (r instanceof Suite) r.init(); });
		} catch (error) {
...
Suite.hooks.BEFORE = 'before';
Suite.hooks.BEFORE_EACH = 'beforeEach';
Suite.hooks.AFTER = 'after';
Suite.hooks.AFTER_EACH = 'afterEach';

Suite.events.EVENT_SUITE_PREPARED = 'suitePrepared';
Suite.events.SUITE_BEGIN = 'suiteBegin';
Suite.events.SUITE_END = 'suiteEnd';
