...
const collector = require('../collectors').Test;
...
module.exports = class Test extends Collectable(Nested(Filterable(Proxy(TaskRunnable(EventEmitter))))) {
	...
	/**
	 * @description
	 * Rendered CallSite-Record
	 * Used when stacktrace of real error cant be recovered
	 * Rendered one points on 'test' function so it's pretty much accurate
	 * @type {*}
	 */
	rendered;
	/**
	...
	constructor({fn, title, runner, parent} = {}, {
		...
		this.rendered = Stacktrace({byFunctionName: 'test'});
		this.collect(collector);
	}

	/**
	 * @description
	 * Runs test in framework state
	 * @override
