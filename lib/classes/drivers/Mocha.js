...
const Mocha = class Mocha extends Driver {
	...
	init(config, runner) {
		...
		this.runner.on(this.runner.constructor.events.TEST_FAILED, (test, error) => {
			this.result(test, Test.states.FAILED, error);
		});
	 ...
	 * @override
	 * @see Driver
	 * @returns {External} - External entity
	 */
	suite = (title, inst) => {
