...
const Runnable = class Runnable extends Exporter {
	...
	constructor({fn, name, context = {}} = {}, config) {
		...
		if (!fn || typeof fn !== 'function')
			throw new TypeError(`${this.constructor.name} fn should be a function`);
		if (config && typeof config !== 'object')
			throw new TypeError(`${this.constructor.name} config should be an object`);
		//Building config
		...
		//Check timeout value
		if (this.config && this.config.timeout && typeof this.config.timeout !== 'number')
	...
	skip() {
		this.emit(this.constructor.events.EVENT_RUNNABLE_SKIP);
	}
	 ...
	 */
	 delayed() {
		...
		if (this.config.timeout > 0)
			return p.timeout(this.config.timeout, new errors.MITimeoutError(this, this.config.timeout));
		return p;
	}
...
Runnable.events.EVENT_RUNNABLE_SKIP = 'skip';
